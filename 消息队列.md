...
为什么选择RocketMQ？因为RocketMQ由阿里开发，不仅文档，示例，功能齐全，语言友好，还与dubbo师出同门，由此入门消息队列相对来说更轻松。
...
**NameServer**是一个功能齐全的服务器，其角色类似Dubbo中的Zookeeper，但NameServer与Zookeeper相比更轻量。主要是因为每个NameServer节点互相之间是独立的，没有任何信息交互。平时主要开销是在维持心跳和提供Topic-Broker的关系数据，但有一点需要注意，Broker向NameServer发心跳时， 会带上当前自己所负责的所有**Topic**信息，如果**Topic**个数太多（万级别），会导致一次心跳中，就Topic的数据就几十M，网络情况差的话， 网络传输失败，心跳失败，导致NameServer误认为Broker心跳失败。
...
![rocketmq_design_1](E:\DistCode\TyporaLoad\消息队列.assets\rocketmq_design_1.png)

### 消息发送和消费的模式

* 通过轮询，**Producer**轮询某个**Topic**下面的所有队列实现发送方的负载均衡

* 消费端会通过**RebalanceService**线程，10秒钟做一次基于**Topic**下的所有队列负载。

消息存储是RocketMQ中最为重要的一部分
...
为了保证消息是肯定被至少消费成功一次，如果这时候消息消费失败，RocketMQ会把这批消息重发回Broker，在延迟到某个时间点后，再次投递到这个ConsumerGroup。而如果一直这样重复消费都持续失败到一定次数（默认16次），就会投递到DLQ死信队列。应用可以监控死信队列来做人工干预。
